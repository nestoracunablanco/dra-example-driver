...
kubectl apply -f "${CURRENT_DIR}/kubevirt-fedora-vm.yaml"
...
kubectl patch vm testvm --type json -p "[
  { \"op\": \"replace\",
...
kubectl wait --for=condition=Ready vm/testvm --timeout=300s

echo "VM is ready, you can login using the command: virtctl ssh -i /tmp/vm_ssh_key -t \"-o StrictHostKeyChecking=no\" test@vm/testvm"